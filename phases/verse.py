...
#
# Single verses only — no "3:5-6" style ranges. The spoken/on-screen citation
# reads as "Book Chapter:Verse" (e.g. "John 1:3"); a range citation ("Psalm
# 62:1-2") reads awkwardly out loud and in captions, so every reference here
# is trimmed to whichever single verse in the passage stands on its own.
VERSE_REFS = [
    "John 3:16", "Psalm 23:1", "Philippians 4:13", "Romans 8:28", "Joshua 1:9",
    "Proverbs 3:5", "Isaiah 41:10", "Jeremiah 29:11", "Psalm 46:1", "Matthew 6:33",
    "Romans 12:2", "Philippians 4:6", "2 Timothy 1:7", "Psalm 27:1", "Isaiah 40:31",
    "Proverbs 16:3", "Psalm 119:105", "Ephesians 6:10", "Ephesians 6:11", "1 Corinthians 16:13",
    "Deuteronomy 31:6", "Psalm 34:18", "Matthew 11:28", "James 1:2", "Romans 8:31",
    "Galatians 5:22", "Psalm 91:1", "Proverbs 18:10", "Isaiah 54:17", "2 Corinthians 5:17",
    "Hebrews 11:1", "Psalm 37:4", "Matthew 5:14", "John 14:6", "John 14:27",
    "Romans 5:8", "1 Peter 5:7", "Psalm 121:1", "Nahum 1:7", "Proverbs 27:17",
    ...
    "Lamentations 3:23", "Micah 6:8", "Nehemiah 8:10", "Psalm 32:8", "Psalm 55:22",
    "Matthew 28:19", "John 8:32", "John 15:5", "Acts 1:8", "Romans 10:9",
    "Romans 12:1", "1 Corinthians 10:13", "1 Corinthians 13:4", "2 Corinthians 12:9", "Galatians 2:20",
    "Ephesians 2:8", "Ephesians 4:32", "Philippians 1:6", "Philippians 2:3", "Philippians 4:8",
    "Colossians 3:2", "1 Thessalonians 5:18", "2 Timothy 4:7", "Hebrews 12:1", "Hebrews 13:5",
    "James 1:5", "James 4:7", "1 Peter 2:9", "1 Peter 4:8", "2 Peter 1:3",
    "1 John 1:9", "1 John 5:4", "Jude 1:25", "Revelation 21:4", "Genesis 1:1",
    "Exodus 14:14", "Numbers 6:24", "Deuteronomy 6:5", "Joshua 24:15", "1 Samuel 17:47",
    "Psalm 1:1", "Psalm 16:11", "Psalm 62:1", "Psalm 100:4", "Proverbs 31:25",
]
...
def fetch_verse_text(reference: str, translation: str = None) -> str:
    """Fetch verse text for `reference` from bible-api.com (free, no-key,
    public-domain-only translations — selectable in Settings via
    Config.VERSE_TRANSLATION: kjv/web/webbe/asv/bbe/darby/dra/oeb-us/oeb-cw/
    ylt). Retries briefly, then falls back to the small embedded set above —
    but ONLY when the requested translation is kjv, since that fallback text
    is hand-typed KJV wording and must never be served as if it were a
    different translation. The API's own docs say it "can and will go down
    from time to time" with no uptime guarantee, so kjv can't have a hard
    dependency on it; other translations have no fallback and fail loudly
    if the API is down for them."""
    translation = (translation or getattr(Config, "VERSE_TRANSLATION", "kjv") or "kjv").lower()
    url = f"https://bible-api.com/{reference.replace(' ', '+')}"
    last_err = None
    for attempt in range(3):
        try:
            r = requests.get(url, params={"translation": translation}, timeout=10)
    ...
    if translation == "kjv" and reference in FALLBACK_VERSES:
        log.warning(f"   Verse API unreachable ({last_err}) — using embedded fallback text for {reference}")
        return FALLBACK_VERSES[reference]
    raise RuntimeError(
        f"Could not fetch verse text for '{reference}' ({translation}) — bible-api.com unreachable "
        f"({last_err})" + ("" if translation != "kjv" else " and no embedded fallback exists for this specific reference")
    )
...
def build_verse_topic(brand=None) -> dict:
    ...
    translation = (getattr(Config, "VERSE_TRANSLATION", "kjv") or "kjv").lower()
    log.info(f"📖 Verse of the Day: {reference} ({translation.upper()})")
    verse_text = fetch_verse_text(reference, translation)
    return {
        "id": f"verse_{int(time.time()*1000)}",
        "idea": f"Verse of the Day: {reference} ({translation.upper()})",
...
def build_verse_script(topic: dict) -> dict:
    ...
    verse_text = topic.get("verse_text") or fetch_verse_text(reference, topic.get("translation"))
    # Citation reads at the END, after the verse — "...he shall direct thy
    # paths. Proverbs 3:5." — not announced up front. `hook`/`reveal` below
    # are unaffected (Scene Engine only pools them for theme-keyword
    # matching, order doesn't matter there); this only reorders the actual
    # spoken/captioned text.
    script_full = f"{verse_text} {reference}." if reference else verse_text
    return {
        "hook": reference,
        "build": "",
        "reveal": verse_text,
