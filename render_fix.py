...
def create_srt(script_text: str, transcription: dict = None) -> str:
    ...
    #
    # Two bugs fixed here (root cause of captions showing a duplicated
    # citation like "55:22 55:22" instead of "Psalm 55:22" once):
    #
    # 1. The "clean" key stripped everything but letters, so a numeric token
    #    like "55:22." collapsed to an EMPTY string. Any other word that also
    #    collapsed to empty (another number, stray punctuation) could then
    #    falsely match it. Digits and colons are now kept in the key so
    #    numeric references compare on their actual content instead of all
    #    looking identical (empty).
    # 2. Nothing stopped the SAME script token from being matched twice. If
    #    Whisper's transcription repeats or hallucinates a word/number (a
    #    known ASR quirk, especially on short digit-heavy audio), the old
    #    code happily inserted that script token into the captions again for
    #    every repeat. `consumed` now marks a script token used the first
    #    time it's matched so a later Whisper repeat can't grab it again.
    enriched = []
    script_idx = 0
    consumed = set()
    for ww in whisper_words:
        raw = ww.get("word", "").strip()
        clean = _re.sub(r'[^a-zA-Z0-9:\']', '', raw).lower()
        ...
        if clean:
            ...
            for si in range(search_start, search_end):
                if si in consumed:
                    continue
                st_clean = _re.sub(r'[^a-zA-Z0-9:\']', '', script_tokens[si]).lower()
                if st_clean == clean:
                    enriched.append({
    ...
    # Collapse back-to-back duplicate words. Whisper occasionally hallucinates
    # or repeats a short/ambiguous word — numbers and references like "55:22"
    # are exactly the audio it does this on most — and the consumed-token
    # tracking above only stops the SCRIPT side from being reused twice; if
    # Whisper itself emitted the same word twice, both copies still reach
    # `enriched` and land in the same caption chunk (the literal "55:22
    # 55:22" bug). Two consecutive identical words within half a second of
    # each other is never intentional speech, so drop the repeat.
    deduped = []
    for w in enriched:
        clean_w = _re.sub(r'[^a-zA-Z0-9\']', '', (w.get("word") or "").strip()).lower()
        if deduped:
            prev = deduped[-1]
            prev_clean = _re.sub(r'[^a-zA-Z0-9\']', '', (prev.get("word") or "").strip()).lower()
            if clean_w and clean_w == prev_clean and (w.get("start", 0) - prev.get("end", 0)) < 0.5:
                continue
        deduped.append(w)
    enriched = deduped
